...
public class Philosopher extends Thread {
	...
	public void test(int test_id){
		if(state[test_id]==HUNGRY && state[(test_id-1+N)%N]!= EATING && state[(test_id+1)%N]!= EATING){
			state[test_id] = EATING;
			table.ForkTake_GUI(test_id);
			sem[test_id].release();
		}
	}
	...
	@Override
	public void run(){
		//hungry- red , eat-blue, think-black&white   -> plates
		//used - red, idle-b&w	-> forks

		Random rand = new Random();
		int waiting_time = rand.nextInt(9000);

		//walk , put plate, wait others, start dining
		...
		//barrier for 5 threads so that they will wait others to arrive to table.
		for(int i = 0; i< N ; i++){
			if(i != id){
				barriers[i].release();
			}
		...
		while(true){
			...
			int thinking_time = rand2.nextInt(10000);

			try {
				sleep(thinking_time); //think
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
